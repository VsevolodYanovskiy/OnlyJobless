...
## Задачи участников:
1. TL: Яновский Всеволод. Задачи: Реализация возможности регистрации пользователей на сервисе для их дальнейшей авторизации. Реализация безопасного хранения паролей в БД. Распределение задач. Составление документации. Ревью кода участников. "Косметические" паравки кода участников. Срок выполнения: 4-5 дней.
...
КОНЕЧНО! Зачем иначе мы бы делали api для разных ллм?

### 5.Как именно бекенд будет общаться с моделькой?

Мы используем только официальные API моделей.

### 6.что по API?

Все просто, наш выбор пал на REST API + WebSocket(для общения с LLM).

### 7.Как же модель поймет что ей делать?

Мы напишем системный промпт, в котором укажем модели быть интервьюером для должности указанной пользователем. Не будем же мы заставлять вас самих просить модель вас интервьюировать, иначе в чем суть проета, duh?

### 8.В каком формате все будет отображаться? Что именно будет видеть пользователь?

Во время загрузки ответа LLM вы будете видеть стандартный для большинсва чат-ботов индикатор. Боковая панель будет так же выглядеть привычно, так же как и в большинстве чат-ботов, например Chat-GPT.

### 9.В каком формате будет оценка?
...
### 10.Влияет ли получение подсказки/ответа на вопрос на итоговую оценку?

Конечно да, это же сохраняется в истории чата, а при выставлении оценки мы попросим модель анализировать весь чат, и конечно дадим ей указание снижать оценку. Очевидно, что нельзя получить 10, пользуясь помощью - это значит что знания не идеальны. 

### 11.Можно ли удалять чаты?

Да, это стандартная практика.

### 12.В каком формате ответ?
...
### 15.Можно ли выбрать уровень сложности?

Конечно, вакансия под собой это и подрузамевает, вы будете проходить собеседование например на middle backend C++ разработчика, а не просто на программиста. Все зависит от того какую выкансию вы укажете в желаемой.

### 16.Можно ли выбрать тип собеседования?
